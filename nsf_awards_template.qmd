...
params:
  program: "[NSF Division of Social and Economic Sciences (SBE/SES)](https://www.nsf.gov/sbe/ses)" # write the title of the program you are analyzing in this format [title](url)
  introduction: "Add introductory text here for the award program"
  csv: "data/awards.csv"
  title_keep_only: "Collaborative Research|other thing you want to keep" # you can add exact wording of what you want to keep in the title, for each new word or phrase add '|' in between. Leave empty (i.e., "") if you don't want to filter based on this.
...
awards <- read_csv(params$csv, locale = locale(encoding = "Latin1"))
```
...
awards <- awards |>
  filter(year(last_amendment_date) >= params$years_keep)
...
awards_state <- awards |>
  mutate(state = organization_state) |>
  group_by(state) |>
  summarise(total_amount = sum(awarded_amount_to_date, na.rm = TRUE))
...
plot_usmap(
  ...
  theme(
    ...
    text = element_text(color = '#A4232B'),
    panel.background = element_rect(fill = "#F4EFE1"),
